                ...
                {props.course?.units.sort((a, b) => a.unitNumber - b.unitNumber).map((unit, i) => (
                    <div key={i} className="flex flex-col gap-3">
                        <h3 className={`text-tertiary-title-size font-semibold text-ellipsis break-words`} >
                            <Link href={`/course/${props.courseId}/unit/${unit.id}`} onClick={() => setIsMenuExpanded(false)} className={`${unit.id === props.selectedUnit ? "text-primary-600 hover:text-primary-300" : "text-slate-600 hover:text-slate-300"}`}>
                        ...
                        {unit.lessons.sort((a, b) => a.lessonNumber - b.lessonNumber).map((lesson, j) => (
                            <div key={j} className="flex flex-col gap-3">
                                <h4 className={`normal-content-size font-semibold   text-ellipsis break-words`} >
                                    <Link href={`/course/${props.courseId}/lesson/${lesson.id}`} onClick={() => setIsMenuExpanded(false)} className={`${lesson.id === props.selectedLesson ? "text-primary-600 hover:text-primary-300" : "text-slate-600 hover:text-slate-300"}`}>
